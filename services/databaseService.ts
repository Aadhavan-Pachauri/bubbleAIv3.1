...
// FIX: Added missing getUserProfile export
...
export const incrementThinkingCount = async (supabase: SupabaseClient, userId: string): Promise<void> => {
    if (userId === 'guest') return; // Skip for guests
...
export const extractAndSaveMemory = async (supabase: SupabaseClient, userId: string, userText: string, aiText: string, projectId?: string | null): Promise<void> => {
    if (userId === 'guest') return; // Skip background memory extraction for guests for now
...
export const loadMemoriesForPrompt = async (supabase: SupabaseClient, userId: string, prompt: string, projectId?: string | null): Promise<string> => {
    ...
    if (userId === 'guest') {
        ...
        const stored = localStorage.getItem('guest_memories');
    ...
    // Map local storage format to DB format if needed (local has 'content' from saveMemory implementation below)
    const normalizedData = data.map(m => ({
        layer: m.layer,
        content: m.content || m.value, // Handle potential schema differences
        metadata: m.metadata
    }));

    const relevantMemories = normalizedData.filter(m => {
    ...
    finalMemories.forEach(m => {
        ...
        if (m.layer && key && value) {
           // Default layer if undefined/unknown
           const targetLayer = byLayer[m.layer] ? m.layer : 'personal'; 
           byLayer[targetLayer as MemoryLayer].push(`[${key}]\n${value}`);
        }
    });
...
export const saveMemory = async (supabase: SupabaseClient, userId: string, layer: MemoryLayer, key: string, value: string, projectId?: string | null): Promise<Memory> => {
    ...
    if (userId === 'guest') {
        ...
        const metadata = { memory_key: key, project_id: projectId };
        
        const existingIndex = memories.findIndex((m: any) => 
            m.layer === layer && m.metadata?.memory_key === key
        );
        ...
        if (existingIndex >= 0) {
            ...
            savedMemory = memories[existingIndex];
        } else {
            savedMemory = {
...
export const getMemoriesForUser = async (supabase: SupabaseClient, userId: string): Promise<Memory[]> => {
    ...
    if (userId === 'guest') {
        ...
        return memories.map((m: any) => ({
            ...m,
            key: m.metadata?.memory_key || '[No Key]',
            value: m.content
        }));
    }
...
export const updateMemory = async (supabase: SupabaseClient, memoryId: string, updates: Partial<Omit<Memory, 'id' | 'user_id' | 'created_at'>>): Promise<Memory> => {
    // --- GUEST HANDLING ---
    if (memoryId.startsWith('guest-mem-')) {
        const memories = JSON.parse(localStorage.getItem('guest_memories') || '[]');
        const index = memories.findIndex((m: any) => m.id === memoryId);
        if (index === -1) throw new Error("Memory not found");
        
        const current = memories[index];
        const newMetadata = { ...current.metadata };
        if (updates.key) newMetadata.memory_key = updates.key;

        const updatedMemory = {
            ...current,
            content: updates.value || current.content,
        ...
        return {
            ...updatedMemory,
            key: updatedMemory.metadata.memory_key,
...
export const deleteMemory = async (supabase: SupabaseClient, memoryId: string): Promise<void> => {
    // --- GUEST HANDLING ---
    if (memoryId.startsWith('guest-mem-')) {
        let memories = JSON.parse(localStorage.getItem('guest_memories') || '[]');
...
export const searchUsers = async (supabase: SupabaseClient, query: string, currentUserId: string): Promise<Profile[]> => {
    if (!query) return [];
    if (currentUserId === 'guest') return []; // Guests can't search db
    const { data, error } = await supabase
        .from('profiles')
        .select('*')
...
export const sendFriendRequest = async (supabase: SupabaseClient, senderId: string, recipientId: string): Promise<void> => {
    if (senderId === 'guest') throw new Error("Guests cannot add friends");
    // Check if exists
    ...
    const { error } = await supabase
        .from('friendships')
        .insert({ user_id: senderId, friend_id: recipientId, status: 'pending' });
...
export const getFriendships = async (supabase: SupabaseClient, userId: string): Promise<Friendship[]> => {
    ...
    // Map to Friendship type expected by UI
    return (data || []).map((f: any) => {
        const isUserInitiator = f.user_id === userId;
        const otherProfile = isUserInitiator ? f.friend : f.user;
        return {
            id: f.id,
            user_id: userId,
...
export const getPendingFriendRequests = async (supabase: SupabaseClient, userId: string): Promise<any[]> => {
    if (userId === 'guest') return [];
    // Incoming requests: user is friend_id, status is pending
    const { data, error } = await supabase
        .from('friendships')
