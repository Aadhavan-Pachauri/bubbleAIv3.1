...
export const ChatMessage: React.FC<ChatMessageProps> = ({ 
  ...
  const handlePlayAudio = async () => {
      ...
      try {
          // Use clean text for speech to avoid reading code blocks/tags
          const speechText = clean.substring(0, 1000); // Limit length for speed
          const base64Audio = await generateSpeech(speechText, geminiApiKey);
          
          const AudioContextClass = window.AudioContext || (window as any).webkitAudioContext;
          const audioCtx = new AudioContextClass();
          
          // Use our manual PCM decoder
          ...
          source.onended = () => {
              setIsPlayingAudio(false);
              setIsGeneratingAudio(false); // Ensure loader stops
          };
          ...
          addToast("Unable to play audio. The model might not support TTS.", "error");
          setIsPlayingAudio(false);
      } finally {
  ...
  return (
    <motion.div variants={variants} initial="hidden" animate="visible" className={`flex items-start gap-4 transition-opacity duration-300 ${isDimmed ? 'opacity-30' : 'opacity-100'} max-w-full`}>
        ...
        <div className={`flex-1 min-w-0 ${isCurrentResult ? 'rounded-lg ring-2 ring-yellow-400' : ''}`}>
            ...
            {/* Thinking Block Display - Visible thought process */}
