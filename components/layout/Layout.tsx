  ...
  const handleLocalSendMessage = async (text: string, files: File[] | null = null, chat?: ChatWithProjectData | null, modelOverride?: string) => {
      try {
          ...
          if (projectUpdate && activeProject) {
               if (isGuest) {
                   // Guests handled by local service in hook, no project update action needed here for now as guests are blocked from projects
        ...
        if (workspaceMode === 'cocreator') {
            if (!activeProject) {
                switch (hubView) {
                    ...
                    default:
                        if (isGuest) {
                             // Show empty or blocked view for guests
